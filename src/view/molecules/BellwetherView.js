...
function BellwetherViewForElection({election, ent}) {
  ...
  const l1Error = resultsForLK.partyToVotes.getL1Error(resultsForEnt.partyToVotes);
  ...
  return (
    <tr >
      ...
      <td style={{color: colorEnt}}>
        {winningPartyEnt}
      </td>
      <td style={{color: colorLK}}>
        {winningPartyLK}
      </td>
      <td style={{color: colorLK}}>
        {isMatch ? "✓" : ""}
      ...
      <table >
        ...
        <tbody>
          {elections.map(function(election, iElection) {
            return <BellwetherViewForElection key={'election-'+iElection} election={election} ent={ent} />;
          })}
        </tbody>
      </table>
    </Box>

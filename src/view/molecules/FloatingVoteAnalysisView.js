...
function getSparseMatrix(partyGroups, elections, ents) {
  ...
  for (let ent of ents) {
    ...
    const electors = elections.filter((e) => !e.isFuture)[0].getResults(ent.id).summary.electors;
...
export default function FloatingVoteAnalysisView({
  ...
  return (
    <SectionBox>
      ...
      <MatrixView sparseMatrix={sparseMatrix} xKey="PartyGroup" yKey="Region" zKey="Base" />
    </SectionBox>
  );
}
